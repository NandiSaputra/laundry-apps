...
@section('title', 'Edit Kategori | ' . ($shopSettings['shop_name'] ?? 'LaundryBiz'))
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Perbarui Kategori</h1>
            <p class="text-sm text-slate-500 dark:text-[#92adc9] mt-1">Ubah informasi kategori layanan laundry Anda.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111a22] rounded-xl border border-slate-200 dark:border-[#324d67] shadow-sm overflow-hidden transition-colors">
        <form id="kategoriForm" action="{{ route('admin.kategori.update', $category->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" placeholder="Jelaskan detail kategori ini..." 
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-[#233648] border border-slate-200 dark:border-[#324d67] rounded-lg focus:ring-2 focus:ring-[#137fec] outline-none text-slate-800 dark:text-white transition-all font-medium">{{ old('deskripsi', $category->deskripsi) }}</textarea>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <div class="flex items-center gap-4 py-2 px-1">
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" name="is_active" value="1" class="accent-[#137fec]" {{ old('is_active', $category->is_active) == '1' ? 'checked' : '' }}>
                    ...
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" name="is_active" value="0" class="accent-red-500" {{ old('is_active', $category->is_active) == '0' ? 'checked' : '' }}>
