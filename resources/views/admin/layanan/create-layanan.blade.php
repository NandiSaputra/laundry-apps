...
@section('title', 'Tambah Layanan Baru | ' . ($shopSettings['shop_name'] ?? 'LaundryBiz'))
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white uppercase tracking-tight">Entry New Service</h1>
            <p class="text-sm text-slate-500 dark:text-[#92adc9] mt-1 font-medium">Definisikan item laundry baru dan tentukan harganya.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111a22] rounded-2xl border border-slate-200 dark:border-[#324d67] shadow-xl overflow-hidden transition-colors">
        <form id="layananForm" action="{{ route('admin.layanan.store') }}" method="POST" class="p-8 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    @error('kategori_id')
                        <p class="text-[9px] text-red-500 mt-1.5 ml-1 font-black uppercase tracking-widest italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="kode_layanan" id="kode_layanan" placeholder="AUTO GENERATED IF EMPTY" 
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-[#233648] border border-slate-200 dark:border-[#324d67] rounded-xl focus:ring-4 focus:ring-[#137fec]/10 outline-none text-slate-800 dark:text-white transition-all font-black tracking-[0.2em] uppercase placeholder:text-slate-300 dark:placeholder:text-[#5a7690] @error('kode_layanan') border-red-500 @enderror"
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <input type="text" name="nama" id="nama" placeholder="E.G. CUCI KILOAN REGULER 2 HARI" 
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-[#233648] border border-slate-200 dark:border-[#324d67] rounded-xl focus:ring-4 focus:ring-[#137fec]/10 outline-none text-slate-800 dark:text-white transition-all font-bold uppercase tracking-tight placeholder:text-slate-300 dark:placeholder:text-[#5a7690] @error('nama') border-red-500 @enderror"
                ...
                <div class="space-y-2">
                    ...
                    <select name="satuan" id="satuan" 
                        ...
                        <option value="pcs" {{ old('satuan') == 'pcs' ? 'selected' : '' }}>PIECE (PCS)</option>
                        <option value="pasang" {{ old('satuan') == 'pasang' ? 'selected' : '' }}>PASANG (SET)</option>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        <input type="number" name="estimasi_jam" id="estimasi_jam" placeholder="24" 
                               ...
                               value="{{ old('estimasi_jam', 24) }}" required>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-6 py-2 px-1">
                        <label class="flex items-center gap-2.5 cursor-pointer group">
                            <input type="radio" name="is_active" value="1" class="w-4 h-4 text-[#137fec] focus:ring-[#137fec]/20" {{ old('is_active', '1') == '1' ? 'checked' : '' }}>
                        ...
                        <label class="flex items-center gap-2.5 cursor-pointer group">
                            <input type="radio" name="is_active" value="0" class="w-4 h-4 text-red-500 focus:ring-red-500/20" {{ old('is_active') == '0' ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" placeholder="DESCRIBE THE SERVICE PARAMETERS..." 
                              class="w-full px-4 py-3 bg-slate-50 dark:bg-[#233648] border border-slate-200 dark:border-[#324d67] rounded-xl focus:ring-4 focus:ring-[#137fec]/10 outline-none text-slate-800 dark:text-white transition-all font-bold uppercase tracking-tight placeholder:text-slate-300 dark:placeholder:text-[#5a7690]">{{ old('deskripsi') }}</textarea>
                </div>
            </div>

            <div class="pt-6 border-t border-slate-50 dark:border-[#324d67]">
                <button type="submit" id="submitBtn" class="w-full py-5 bg-[#137fec] text-white rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] hover:bg-[#137fec]/90 shadow-2xl shadow-[#137fec]/30 transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed group">
                    <span class="material-symbols-outlined group-hover:rotate-90 transition-transform">add_task</span>
                    <span>Commit Service Addition</span>
                </button>
            </div>
        </form>
    </div>
