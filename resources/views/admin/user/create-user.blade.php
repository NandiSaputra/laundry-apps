...
@section('title', 'Tambah User Baru | ' . ($shopSettings['shop_name'] ?? 'LaundryBiz'))
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Buat Akun Baru</h1>
            <p class="text-sm text-slate-500 dark:text-[#92adc9] mt-1">Daftarkan staf atau admin baru ke dalam sistem.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111a22] rounded-xl border border-slate-200 dark:border-[#324d67] shadow-sm overflow-hidden transition-colors">
        <form id="userForm" action="{{ route('admin.user.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="nama" id="nama" placeholder="Masukkan nama staf..." 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    <label for="email" class="text-xs font-bold text-slate-500 dark:text-[#92adc9] ml-1 uppercase tracking-wider">Email (Login ID)</label>
                    <input type="email" name="email" id="email" placeholder="contoh@laundrybiz.com" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1.5">
                    ...
                    <select name="role" id="role" 
                        ...
                        <option value="produksi" {{ old('role') == 'produksi' ? 'selected' : '' }}>Staf Produksi (Cuci/Setrika)</option>
                ...
                <div class="space-y-1.5">
                    <label for="password" class="text-xs font-bold text-slate-500 dark:text-[#92adc9] ml-1 uppercase tracking-wider">Password</label>
