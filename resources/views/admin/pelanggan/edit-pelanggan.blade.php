...
@section('title', 'Edit Pelanggan | ' . ($shopSettings['shop_name'] ?? 'LaundryBiz'))
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Data Pelanggan</h1>
            <p class="text-sm text-slate-500 dark:text-[#92adc9] mt-1">Perbarui informasi member setia Anda.</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-[#111a22] rounded-xl border border-slate-200 dark:border-[#324d67] shadow-sm overflow-hidden transition-colors">
        <form id="pelangganForm" action="{{ route('admin.pelanggan.update', $pelanggan->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="alamat" id="alamat" rows="3" placeholder="Masukkan alamat pelanggan..." 
                          class="w-full px-4 py-2.5 bg-slate-50 dark:bg-[#233648] border border-slate-200 dark:border-[#324d67] rounded-lg focus:ring-2 focus:ring-[#137fec] outline-none text-slate-800 dark:text-white transition-all font-medium">{{ old('alamat', $pelanggan->alamat) }}</textarea>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 dark:border-[#324d67]">
                <button type="submit" id="submitBtn" class="w-full py-4 bg-[#137fec] text-white rounded-lg font-bold hover:bg-[#137fec]/90 shadow-lg shadow-[#137fec]/20 transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-xs">
                    ...
                    <span>Update Data Pelanggan</span>
                </button>
            </div>
        </form>
    </div>
