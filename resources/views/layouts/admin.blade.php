...
<head>
    ...
    <title>@yield('title', 'Laundry Management Dashboard | ' . ($shopSettings['shop_name'] ?? 'LaundryBiz'))</title>
    ...
    <style>
        ...
        /* Custom Colors based on LaundryBiz */
        :root {
            --primary: #137fec;
        ...
        /* Force Primary Color */
        .text-indigo-600, .text-indigo-500 { color: #137fec !important; }
        .bg-indigo-600, .bg-indigo-500 { background-color: #137fec !important; }
        .bg-indigo-50 { background-color: rgba(19, 127, 236, 0.05) !important; }
        .border-indigo-100 { border-color: rgba(19, 127, 236, 0.1) !important; }
        .hover\:bg-indigo-600:hover { background-color: #0b93ff !important; }
...
<body class="bg-slate-50 dark:bg-[#101922] text-slate-900 dark:text-slate-100 font-display transition-colors duration-200">
    ...
    <div id="global-loader" style="display: none;" class="fixed inset-0 z-9999 place-items-center bg-white/70 dark:bg-slate-900/70 transition-all duration-300 opacity-0 pointer-events-none">
        <div class="flex flex-col items-center">
            ...
            <p class="mt-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Memuat Data...</p>
        </div>
    </div>
        ...
        <main class="flex-1 flex flex-col overflow-y-auto">
            ...
            <header class="flex items-center justify-between border-b border-slate-200 dark:border-[#233648] px-4 sm:px-8 py-3 sm:py-4 bg-white dark:bg-[#111a22] sticky top-0 z-40 transition-colors duration-200">
                <div class="flex items-center gap-3 sm:gap-6">
                    <button @click="sidebarOpen = !sidebarOpen" class="p-2 rounded-lg text-slate-500 hover:bg-slate-50 dark:hover:bg-[#233648] transition-colors active:scale-95">
                        <span class="material-symbols-outlined text-2xl">menu</span>
                    </button>
                    <h2 class="text-base sm:text-xl font-bold tracking-tight text-slate-800 dark:text-white truncate max-w-[150px] sm:max-w-none">@yield('page-title', 'Ikhtisar')</h2>
                </div>
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    <div x-data="{ userMenu: false }" class="relative">
                        <button @click="userMenu = !userMenu" class="flex items-center gap-2 p-1 sm:p-1.5 rounded-xl hover:bg-slate-50 dark:hover:bg-[#233648] transition-all active:scale-95">
                            ...
                            <span class="hidden sm:inline text-sm font-bold text-slate-700 dark:text-gray-200">{{ Auth::user()->nama }}</span>
                            <span class="material-symbols-outlined text-slate-400 text-lg hidden sm:inline">expand_more</span>
                        </button>
                        ...
                        <div x-show="userMenu" @click.away="userMenu = false" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-[#111a22] border border-slate-200 dark:border-[#233648] rounded-xl shadow-2xl z-50 overflow-hidden" x-cloak>
                            <div class="px-4 py-3 border-b border-slate-50 dark:border-[#233648] sm:hidden">
                                ...
                                <p class="text-[10px] text-slate-400 uppercase font-black truncate">{{ Auth::user()->role }}</p>
                            </div>
                            <a href="{{ route(Auth::user()->role . '.profile') }}" class="flex items-center px-4 py-2.5 text-xs sm:text-sm text-slate-600 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-[#233648]">Profil Saya</a>
                            <button onclick="showLogoutModal()" class="w-full text-left flex items-center px-4 py-2.5 text-xs sm:text-sm text-red-600 hover:bg-slate-50 dark:hover:bg-[#233648] font-bold">Keluar</button>
                        </div>
                    </div>
                </div>
            </header>
